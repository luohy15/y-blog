...
24. As a desktop reader, I want both sidebars, the related-articles rail and the table of contents, to scroll without showing a scrollbar, so that they read as clean lists, while the page keeps its normal scrollbar.
...
31. As a mobile reader, I want both floating panels, related articles and the table of contents, to scroll by touch without a visible scrollbar, so that they match the desktop sidebars, while the page scrollbar is unchanged.
...
- **Routes.** A Tags page and a per-tag selected state are added alongside the existing `/writing` shape, including the language-prefixed variant. The per-tag URL renders the Tags page with that tag selected; there is no separate standalone tag page. No existing route, legacy one- or two-segment path, dated path, or slug redirect changes; every existing article URL is preserved byte for byte. The canonical per-tag URL is human-readable: the tag text itself is the path segment, so the travel tag is `/tags/travel` and its Chinese listing is `/zhs/tags/travel`. The router decodes the segment exactly once and matches it against the tags exposed by the index; there is no slug normalisation, no collision handling and no internally generated alias. Two kinds of already-published tag links are honoured by narrowly bounded legacy redirects, each replacing (not pushing) the history entry with the readable canonical URL in the same language. First, an earlier candidate encoded the segment as `t.` plus UTF-8 base64url (`/tags/t.dHJhdmVs`) and was published briefly; a segment in that form that decodes to a current tag redirects to that tag's readable URL. Second, the retired agent tags: a segment equal to `y-agent` or `ai-coding`, whether readable or in the encoded form, redirects to `/tags/ai-agent` in the same language. Both redirects are gated on the target tag actually existing in the tags loaded for the URL's language, so a language whose index lacks the tag falls through rather than redirecting to an empty page. Nothing else is redirected, the old names are never treated as live tags, and every href the site emits uses only current canonical readable URLs. Any other segment that matches no tag (unknown text, malformed encoded form, encoded form decoding to an unknown tag) selects no tag and renders the Tags page's empty-tag state (the "no articles with this tag" copy plus the link back to all tags) without throwing. There is no separate not-found page for tag segments. The opaque encoding was dropped because it over-engineered hypothetical special-character tags; the actual vocabulary is naturally URL-safe.
...
- **Sidebar scrollbars.** Every bounded navigation container on the article page hides its visible scrollbar while remaining fully scrollable: the desktop related-articles rail, the desktop table of contents, the mobile related-articles panel and the mobile table-of-contents panel. Overflow scrolling, the height bound, wheel, touch and keyboard scrolling, focus visibility and heading navigation are all retained, so this is not `overflow: hidden`. One minimal scoped CSS utility, covering Firefox and WebKit/Blink, is shared by all four containers rather than duplicated per container. The page and every other scrollbar keep their default appearance. This supersedes the earlier decision that hid only the related-articles scrollbar and left the TOC's visible; the author asked for the TOC to match after seeing the shipped result.
...
- Test the readable tag URL for each current tag (`travel`, `ai-agent`): tag text to canonical href and href segment back to the tag, with and without a language prefix, and that language switching on a selected tag keeps the same tag selected.
- Test the bounded legacy redirects: an encoded `t.`-prefixed segment for a current tag resolves to the readable canonical URL in the same language as a replace; readable and encoded `y-agent` and `ai-coding` segments resolve to `ai-agent` in the same language as a replace when that language's loaded tags include `ai-agent`, and fall through to the empty-tag state when they do not; an encoded segment for an unknown tag and a malformed encoded segment both fall through to the empty-tag state; and no href produced anywhere uses the encoded form or a retired tag name.
