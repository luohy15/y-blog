    ...
    const handleClick = onClick && typeof onClick === 'string'
      ? () => {
          try {
            ...
            eval(onClick as unknown as string);
          } catch (error) {
...
function parseShortcodes(content: string): string {
  ...
  processedContent = processedContent.replace(rawhtmlRegex, (_match, htmlContent) => {
  ...
  processedContent = processedContent.replace(jsonDisplayRegex, (_match, url) => {
